...
def create_default_windows_sections(apps, schema_editor):
    sections = [
        ...
        },
        {
            ...
            "retrieval_method": "PW",
            "retrieval_output": "JSON",
            "target": "Get-NetAdapter | ForEach-Object {$adapter = $_; $config = Get-CimInstance -ClassName Win32_NetworkAdapterConfiguration | Where-Object {$_.Index -eq $adapter.ifIndex}; [PSCustomObject]@{Description = $_.InterfaceDescription; Type = $_.ifAlias; Speed = $_.LinkSpeed; MTU = $config.MTU; MACAddress = $config.MACAddress; Status = $_.Status; IPAddress = $config.IPAddress; NetMask = $config.IPSubnet; Gateway = $config.DefaultIPGateway; NetworkNumber = if ($config.IPAddress -and $config.IPSubnet) {[System.Net.IPAddress]::new([byte[]](0..3 | ForEach-Object {[System.Net.IPAddress]::Parse($config.IPAddress[0]).GetAddressBytes()[$_] -band [System.Net.IPAddress]::Parse($config.IPSubnet[0]).GetAddressBytes()[$_]})).ToString()} else {$null}; DHCPServer = $config.DHCPServer}} | ConvertTo-Json",
            "options": {},
            "template": apps.get_model("template", "Template").objects.get(os="WIN"),
        },
        {
        ...
        },
        {
            ...
            "retrieval_method": "PW",
            "retrieval_output": "JSON",
            "target": "Get-ItemProperty HKLM:\\Software\\Wow6432Node\\Microsoft\\Windows\\CurrentVersion\\Uninstall\\*, HKLM:\\Software\\Microsoft\\Windows\\CurrentVersion\\Uninstall\\* | Where-Object {$_.DisplayName -and $_.SystemComponent -ne 1} | Select-Object *, @{Name = 'LanguageName'; Expression = {try {if ($_.Language) {$lcid = [int]$_.Language; (New-Object System.Globalization.CultureInfo $lcid).DisplayName} else {'Unknown'}} catch {'Unknown'}}}, @{Name = 'InstallDateFormatted'; Expression = {if ($_.InstallDate -match '^\d{8}$') {[datetime]::ParseExact($_.InstallDate, \"yyyyMMdd\", $null).ToString(\"yyyy/MM/dd\")} else {'Unknown'}}}, @{Name = 'Major'; Expression = {($_.DisplayVersion -split '\.')[0]}}, @{Name = 'Minor'; Expression = {($_.DisplayVersion -split '\.')[1]}} | ConvertTo-Json",
            "options": {},
            "template": apps.get_model("template", "Template").objects.get(os="WIN"),
        },
        {
