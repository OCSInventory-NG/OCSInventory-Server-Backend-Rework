...
def create_default_linux_sections(apps, schema_editor):
    ...
    sections = [
        ...
        },
        {
            ...
            "retrieval_method": "BASH",
            "retrieval_output": "REGX",
            "target": r'for f in /sys/class/drm/*/edid; do [ -f "$f" ] || continue; echo "Screen: $(basename "$(dirname "$f")")"; cat "$f" | parse-edid 2>&1 | sed -nE \'s/.*week ([0-9]+) of ([0-9]+).*/Manufactured week/year: \\1/\\2/p; t; p\'; serial=$(hexdump -v -e \'1/1 "%02x"\' "$f" | cut -c25-32); [ -n "$serial" ] && echo "Serial number: $((0x$serial))"; echo "---"; done',
            "options": {"separator": "---", "multiple": False},
        },
        {
            "name": "PORTS",
        ...
        },
        {
            ...
            "retrieval_method": "BASH",
            "retrieval_output": "REGX",
            "target": r'for name in $(lpstat -p 2>/dev/null | awk \'{print $2}\'); do description=$(lpstat -l -p "$name" | grep "Description" | cut -d: -f2- | sed \'s/^ *//\'); port=$(lpstat -v "$name" | awk -F\': \' \'{print $2}\'); driver=$(lpoptions -p "$name" | grep "printer-make-and-model" | cut -d"\'" -f2); echo "Name: $name"; echo "Description: $description"; echo "Port: $port"; echo "Driver: $driver"; echo "---"; done',
            "options": {"separator": "---", "multiple": False},
        },
        {
            "name": "SLOTS",
        ...
        },
        {
            ...
            "retrieval_method": "BASH",
            "retrieval_output": "REGX",
            "target": r'videos=($(lspci | grep -iE \'graphics|vga|video|display\' | cut -d \' \' -f1)); for slot in "${videos[@]}"; do info=$(lspci -vmm -s "$slot"); chipset=$(echo "$info" | grep \'^Class:\' | cut -f2-); vendor=$(echo "$info" | grep \'^Vendor:\' | cut -f2-); device=$(echo "$info" | grep \'^Device:\' | cut -f2-); name="$vendor $device"; memory=0; while IFS= read -r line; do if [[ $line =~ Memory.*\\(.*-bit,\\ prefetchable\\)\\ \\[size=([0-9]+)([GMK])\\] ]]; then size=${BASH_REMATCH[1]}; unit=${BASH_REMATCH[2]}; case $unit in G) bytes=$(( size * 1024 * 1024 * 1024 )) ;; M) bytes=$(( size * 1024 * 1024 )) ;; K) bytes=$(( size * 1024 )) ;; esac; memory=$(( memory + bytes )); fi; done < <(lspci -v -s "$slot" 2>/dev/null); memory_mb=$(( (memory + 524288) / 1048576 )); echo "Chipset: $chipset"; echo "Name: $name"; echo "Memory: ${memory_mb} MB"; echo "---"; done',
            "options": {"separator": "---", "multiple": False},
        },
        {
