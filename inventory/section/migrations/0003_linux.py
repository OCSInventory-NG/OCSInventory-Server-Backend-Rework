...
def create_default_linux_sections(apps, schema_editor):
    ...
    sections = [
        ...
        },
        {
            ...
            "retrieval_method": "BASH",
            "retrieval_output": "REGX",
            "target": 'chassis_code=$(cat /sys/class/dmi/id/chassis_type); case $chassis_code in 1) chassis="Other" ;; 2) chassis="Unknown" ;; 3) chassis="Desktop" ;; 4) chassis="Low Profile Desktop" ;; 5) chassis="Pizza Box" ;; 6) chassis="Mini Tower" ;; 7) chassis="Tower" ;; 8) chassis="Portable" ;; 9) chassis="Laptop" ;; 10) chassis="Notebook" ;; 11) chassis="Hand Held" ;; 12) chassis="Docking Station" ;; 13) chassis="All in One" ;; 14) chassis="Sub Notebook" ;; 15) chassis="Space-Saving" ;; 16) chassis="Lunch Box" ;; 17) chassis="Main Server Chassis" ;; 18) chassis="Expansion Chassis" ;; 19) chassis="Sub Chassis" ;; 20) chassis="Bus Expansion Chassis" ;; 21) chassis="Peripheral CHassis" ;; 22) chassis="RAID Chassis" ;; 23) chassis="Rack Mount Chassis" ;; 24) chassis="Sealed-case PC" ;; 25) chassis="Multi-System" ;; 26) chassis="CompactPCI" ;; 27) chassis="AdvancedTCA" ;; 28) chassis="Blade" ;; 29) chassis="Blade Enclosing" ;; 30) chassis="Tablet" ;; 31) chassis="Convertible" ;; 32) chassis="Detachable" ;; 33) chassis="IoT Gateway" ;; 34) chassis="Embedded PC" ;; 35) chassis="Mini PC" ;; 36) chassis="Stick PC" ;; *) chassis="$chassis_code" ;; esac; printf "%-25s : %s\\n%-25s : %s\\n%-25s : %s\\n%-25s : %s\\n%-25s : %s\\n%-25s : %s\\n%-25s : %s\\n%-25s : %s\\n%-25s : %s\\n%-25s : %s\\n%-25s : %s\\n" "SystemManufacturer" "$(cat /sys/class/dmi/id/sys_vendor)" "SystemModel" "$(cat /sys/class/dmi/id/product_name)" "SystemSerialNumber" "$(cat /sys/class/dmi/id/product_serial)" "ChassisType" "$chassis" "BiosManufacturer" "$(cat /sys/class/dmi/id/bios_vendor)" "BiosVersion" "$(cat /sys/class/dmi/id/bios_version)" "BiosDate" "$(cat /sys/class/dmi/id/bios_date)" "AssetTag" "$(cat /sys/class/dmi/id/chassis_asset_tag)" "MotherboardManufacturer" "$(cat /sys/class/dmi/id/board_vendor)" "MotherboardModel" "$(cat /sys/class/dmi/id/board_name)" "MotherboardSerialNumber" "$(cat /sys/class/dmi/id/board_serial)"',
            "options": {},
        },
        {
            "name": "CONTROLLERS",
