...
def unseed_mappings(apps, schema_editor):
    ...
    WindowsBuildMapping.objects.filter(
        build__in=[b for b, _ in INITIAL_MAPPINGS]
    ).delete()
        ...
        migrations.CreateModel(
            ...
            fields=[
                ...
                (
                    "build",
                    models.IntegerField(
                        help_text="Numéro de build Windows (ex: 22621)", unique=True
                    ),
                ),
                (
                    "channel",
                    models.CharField(
                        help_text="Canal endoflife.date (ex: 22h2)", max_length=20
                    ),
                ),
            ],
